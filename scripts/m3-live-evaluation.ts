...
try {
  ...
  const result = {
    ...
    evidence: {
      ...
      contexts: Number(game.db.prepare("SELECT COUNT(*) AS count FROM team_context_refs WHERE run_id = ?").get(runId)?.count ?? 0),
      ...
      effects: Number(game.db.prepare("SELECT COUNT(*) AS count FROM team_effects WHERE run_id = ?").get(runId)?.count ?? 0),
      dispatches: Number(game.db.prepare("SELECT COUNT(*) AS count FROM team_dispatches WHERE run_id = ?").get(runId)?.count ?? 0),
      observations: Number(game.db.prepare("SELECT COUNT(*) AS count FROM team_observations WHERE run_id = ?").get(runId)?.count ?? 0),
      ...
      allEffectsVerified: Number(game.db.prepare("SELECT COUNT(*) AS count FROM team_effects WHERE run_id = ? AND status != 'succeeded'").get(runId)?.count ?? 0) === 0,
      allDispatchesSucceeded: Number(game.db.prepare("SELECT COUNT(*) AS count FROM team_dispatches WHERE run_id = ? AND status != 'succeeded'").get(runId)?.count ?? 0) === 0,
