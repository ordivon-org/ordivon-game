...
export class HermesCliProvider implements OperationProvider {
  ...
  constructor(options: HermesCliProviderOptions = {}) {
    ...
    this.providerId = `hermes-cli-isolated-v1:${this.provider}/${this.model}`;
  }
  ...
  async decide(context: CompiledAgentContext): Promise<OperationDecision> {
    const root = mkdtempSync(join(tmpdir(), "ordivon-game-hermes-"));
    ...
    try {
      ...
      for (const name of ["HERMES_INFERENCE_MODEL", "HERMES_INFERENCE_PROVIDER", "HERMES_IGNORE_USER_CONFIG", "HERMES_SAFE_MODE"]) {
        delete env[name];
      }
      ...
      try {
        ...
        throw new ProviderAdapterError("invalid_output", "Hermes returned invalid Decision or usage JSON", { cause: error });
      }
  ...
  private writeCredentials(destination: string): void {
    ...
    catch (error) { throw new ProviderAdapterError("unavailable", "Hermes credential environment file is unavailable", { cause: error }); }
    ...
    if (keyLines.length === 0) throw new ProviderAdapterError("unavailable", `Hermes credential file has no ${keyName}`);
  ...
  private validateUsage(usage: Record<string, unknown>): void {
    if (usage.model !== this.model || usage.provider !== this.provider) {
      throw new ProviderAdapterError("invalid_usage", "Hermes used another model or provider");
    }
    if (usage.completed !== true || usage.failed !== false || Number(usage.api_calls ?? 0) < 1) {
      throw new ProviderAdapterError("invalid_usage", "Hermes usage reports an incomplete or non-model invocation");
    }
  }
}
