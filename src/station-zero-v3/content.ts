...
export const STATION_ZERO_V3_P0_CONTRACT: StationZeroP0Contract = {
  schemaVersion: 1,
  kind: "ordivon.game.station-zero-v3-p0-contract",
  scenario: {
    id: STATION_ZERO_V3_SCENARIO_ID,
    version: STATION_ZERO_V3_SCENARIO_VERSION,
    rulesetId: STATION_ZERO_V3_RULESET_ID,
    rulesetVersion: STATION_ZERO_V3_RULESET_VERSION,
    worldSchemaVersion: STATION_ZERO_V3_WORLD_SCHEMA_VERSION,
  },
  productForm: "single-player-asymmetric-turn-based-tactical-encounter",
  defaultPlayerFactionId: "rescue",
  turnPhases: [...STATION_ZERO_TURN_PHASES],
  resolutionPhases: [...STATION_ZERO_RESOLUTION_PHASES],
  playerResponsibilities: [
    "allocate command points and scarce commander abilities",
    "set objectives, priority targets, protection relationships, and retreat thresholds",
    "choose what information to reveal and which system to manipulate",
    "commit one complete friendly plan each turn",
    "accept casualties, lost objectives, partial success, or extraction",
  ],
  agentResponsibilities: [
    "interpret faction knowledge and standing orders",
    "choose bounded legal movement, combat, interaction, and guard intents",
    "coordinate local execution without inventing world state or rules",
    "adapt to invalidated actions through later turns rather than rewriting committed history",
  ],
  randomnessPolicy: {
    setupMayVary: true,
  ...
  influences: [
    {
      influenceId: "roguelite",
      retainedMechanics: [
        "bounded encounter rewards",
        "build-relevant equipment and abilities",
        "partial success and costly extraction",
        "content contracts that can later feed a branching run",
      ],
      rejectedMechanics: [
        "random hit resolution inside a committed turn",
        "permanent stat inflation in the encounter contract",
        "route-map and meta-progression before the encounter is fun",
      ],
    },
    {
      influenceId: "tactical-rpg",
      retainedMechanics: [
        "position, cover, initiative, equipment, injuries, and faction objectives",
        "telegraphed bounded actions and deterministic resolution phases",
        "persistent named actors whose survival and equipment matter",
      ],
      rejectedMechanics: [
        "player micromanagement of every ordinary step",
        "one global success predicate for asymmetric factions",
        "rollback of an entire turn because one combat intent was invalidated",
      ],
    },
    {
      influenceId: "sandbox",
      retainedMechanics: [
        "material world objects that can be moved, consumed, powered, damaged, looted, and blocked",
        "multiple uses for space, equipment, passages, systems, and hazards",
        "world state rather than authored prose as the source of consequence",
      ],
      rejectedMechanics: [
        "unbounded construction or arbitrary block placement in the first encounter",
        "free-form generated rules",
        "infinite world scope",
      ],
    },
    {
      influenceId: "systemic-sim",
      retainedMechanics: [
        "coupled power, oxygen, heat, alert, biomass, and passage state",
        "several valid responses to the same crisis",
        "environment resolution after faction actions",
      ],
      rejectedMechanics: [
        "real-time control",
        "opaque probability as the main source of failure",
        "simulation detail without a player decision attached",
      ],
    },
    {
      influenceId: "character-sim",
      retainedMechanics: [
        "traits, roles, equipment, injury, capture, death, and faction allegiance",
        "leader Agents controlling lower-cost policy units",
        "histories that can later affect a multi-encounter run",
      ],
      rejectedMechanics: [
        "hundreds of full-fidelity Agent calls in one encounter",
        "relationship simulation before combat identity is proven",
        "dialogue without mechanical consequence",
      ],
    },
  ],
  nonGoals: [
    "real-time combat",
    "human multiplayer",
    "a general RPG engine",
    "an open galaxy",
    "a branching roguelite route map",
    "cross-run progression",
    "a one-hundred-Agent society simulation",
    "player access to hidden world truth",
  ],
};
