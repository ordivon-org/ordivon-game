...
export function renderObjectives(view) {
  const nodes = view.objectives.map((objective) => `
    <article class="objective ${escapeHtml(objective.status)} ${escapeHtml(objective.priority)}">
      <header><span>${objective.status === "satisfied" ? "✓" : objective.status === "active" ? "▶" : "○"}</span><strong>${escapeHtml(objective.label)}</strong></header>
  ...
  return `<section class="panel objective-panel"><div class="section-heading"><div><p class="eyebrow">MISSION GRAPH</p><h2>Verified rescue objectives</h2></div><span>${view.mission.objectiveProgress.satisfied}/${view.mission.objectiveProgress.total}</span></div><div class="objective-grid">${nodes}</div></section>`;
}
