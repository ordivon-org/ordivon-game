---
schema_version: 1
id: game.research.core.product-direction
title: Ordivon Game — Product / Direction Research
profile: research
lifecycle: active
source_role: canonical-navigation
visibility: public
owners:
  - ordivon-game
updated: 2026-08-18
---
# Ordivon Game — Product / Direction Research

This lane is deliberately separate from semantic Foundations.

`GAME_CORE_*`, Station Zero research treatments, Casefile, Concept Lab, and related direction experiments may produce product/design evidence. They do **not** vote on, renumber, or overwrite GDF0–GDF3.

## Current product authority

- [Product](../../docs/PRODUCT.md)
- [Development Model](../../docs/DEVELOPMENT_MODEL.md)
- [Development Core Responsibility Model](../../docs/GAME_DEVELOPMENT_CORE.md)
- [Game Development Paradigm Research](../../docs/GAME_DEVELOPMENT_PARADIGM_RESEARCH.md)
- [Authority](../../docs/authority.md)

## Game Core direction research

- [Game Development Core](../../docs/GAME_DEVELOPMENT_CORE.md)
- [Development Paradigm Research](../../docs/GAME_DEVELOPMENT_PARADIGM_RESEARCH.md)
- [Development Core Case Pressure Tests](../../docs/GAME_DEVELOPMENT_CASE_PRESSURE_TESTS.md)
- [Content / Progression Architecture](../../docs/GAME_CONTENT_PROGRESSION_ARCHITECTURE.md)
- [Player Evidence Programme](../../docs/GAME_PLAYER_EVIDENCE_PROGRAMME.md)
- [Production Agent Environment Comparison](../../docs/GAME_PRODUCTION_AGENT_ENVIRONMENT.md)
- [Game Core Research Reset](../../docs/GAME_CORE_RESEARCH_RESET.md)
- [Game Core Direction Space](../../docs/GAME_CORE_DIRECTION_SPACE.md)
- [Game Core Experiment Findings](../../docs/GAME_CORE_EXPERIMENT_FINDINGS.md)
- [Game Core Experiment — Casefile](../../docs/GAME_CORE_EXPERIMENT_CASEFILE.md)

## Station Zero research / reference treatments

- [Station Zero v3 Product reference](../../docs/STATION_ZERO_V3_PRODUCT.md)
- [Station Zero v3 Product Value](../../docs/STATION_ZERO_V3_PRODUCT_VALUE.md)
- [Station Zero v3 Domain Value](../../docs/STATION_ZERO_V3_DOMAIN_VALUE_GV.md)
- [Station Zero v3 Contraction](../../docs/STATION_ZERO_V3_CONTRACTION.md)
- [Station Zero v3 DeepSeek Pool](../../docs/STATION_ZERO_V3_DEEPSEEK_POOL.md)
- [Station Zero v3 Vertical Slice](../../docs/STATION_ZERO_V3_VERTICAL_SLICE.md)
- [Station Zero v3 P0](../../docs/STATION_ZERO_V3_P0.md)
- [Station Zero v3 P1](../../docs/STATION_ZERO_V3_P1.md)
- [Station Zero v3 P2](../../docs/STATION_ZERO_V3_P2.md)
- [Station Zero v3 P3](../../docs/STATION_ZERO_V3_P3.md)

## Separation rule

```text
Semantic Foundation truth → Current Foundations lane
Product/direction evidence → this lane
Executable product authority → PRODUCT / ARCHITECTURE / DEVELOPMENT_MODEL / authority surfaces
```

Technical maturity, treatment survival, or product usefulness does not imply Foundation admission.
