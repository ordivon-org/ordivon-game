...
test("one Replay Projection restores every revision exactly once",async()=>{const{store,runId}=await run("baseline","security-contain");try{const terminalRevision=store.loadState(runId).revision;const original=store.stateAtRevision.bind(store);let calls=0;(store as any).stateAtRevision=(revision:number,selectedRunId?:string)=>{calls+=1;return original(revision,selectedRunId);};const projection=buildReplayProjection(store,runId);assert.equal(calls,terminalRevision+1);assert.equal(projection.frames.length,terminalRevision+1);assert.equal(diagnoseProjection(store,projection).graphDigest,projection.graph.graphDigest);}finally{store.close();}});
